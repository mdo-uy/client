...
function UsersActive(props) {
    ...
    const editUser = user => {
        setIsVisibleModal(true);
        setModalTitle(`Editar ${user.name ? user.name : 'Nombre sin definir'} ${user.lastname ? user.lastname : 'Apellido sin definir'}`);
        setModalContent(<EditUserForm user={user} setIsVisibleModal={setIsVisibleModal}  setReloadUsers={setReloadUsers}/>);
    }

    return (
        <List className='users-active' itemLayout='horizontal' dataSource={usersActive} renderItem={user => <UserActive user={user} editUser={editUser} setReloadUsers={setReloadUsers} /> } />
    )
}
        ...
        <List.Item 
            ...
            >
                <List.Item.Meta avatar={ <Avatar src={avatar ? avatar : NoAvatar} /> } 
                    title={ `
                        ${user.name ? user.name : 'Agregue su nombre'}
                        ${user.lastname ? user.lastname : 'Agregue su apellido'}
                    `} 
...
function UsersInactive(props) {
    ...
    return (
        <List className='users-active' itemLayout='horizontal' dataSource={usersInactive} renderItem={user => ( <UserInactive user={user} setReloadUsers={setReloadUsers}/> )} />
    )
}
